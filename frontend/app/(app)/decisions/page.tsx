...
import { Check, X, Shield, Brain, Target, AlertTriangle, MessageSquare, Briefcase, Loader2, ShieldCheck, ShieldAlert } from 'lucide-react';
import { useAppStore } from '@/lib/store';
import { fetchDecisionSupportData, submitHiringDecision, DecisionSupportData } from '@/lib/api/decisions';
import { runBiasAudit, BiasAuditResult, BiasFlag } from '@/lib/api/bias_audit';
...
export default function DecisionsPage() {
    ...
    // Reset bias audit when candidate changes
    useEffect(() => {
        setBiasAudit(null);
    }, [selectedCandidateId]);
    ...
    const handleBiasAudit = async () => {
        ...
        try {
            const result = await runBiasAudit(
                selectedCandidateId,
                selectedCandidate?.jobId && selectedCandidate.jobId !== 'unknown'
                    ? selectedCandidate.jobId
                    : undefined,
            );
            setBiasAudit(result);
        } catch {
            // Silently fall through — bias audit failure should not block decisions
        } finally {
                            ...
                            <div className="grid md:grid-cols-2 gap-6">
                                ...
                                <Card>
                                    ...
                                    <CardContent className="space-y-4 pt-4">
                                        ...
                                        <div className="bg-amber-50 border border-amber-100 p-4 rounded-lg mt-6 space-y-3">
                                            <div className="flex items-center justify-between">
                                                <h4 className="text-xs font-bold text-amber-800 flex items-center gap-2">
                                                    <Shield size={14} /> Fairness & Bias Audit
                                                </h4>
                                                {!biasAudit && (
                                                    <button
                                                        onClick={handleBiasAudit}
                                                        disabled={biasLoading}
                                                        className="flex items-center gap-1.5 text-[11px] font-semibold text-indigo-600 hover:text-indigo-800 disabled:opacity-50"
                                                    >
                                                        {biasLoading
                                            ...
                                            {!biasAudit && !biasLoading && (
                                                <p className="text-amber-700/80 text-xs leading-relaxed italic">
                                                    Run the Bias & Fairness Audit to detect demographic proxy signals,
                                                    prestige bias, and exclusionary language in the job description.
                                                </p>
                                            )}

                                            {biasLoading && (
                                                <div className="flex items-center gap-2 text-xs text-amber-700">
                                                    <Loader2 size={13} className="animate-spin" /> Scanning for bias signals…
                                                </div>
                                            )}
                                            ...
                                            {biasAudit && (
                                                <div className="space-y-3">
                                                    {/* Score bar */}
                                                    <div className="flex items-center gap-3">
                                                        {biasAudit.overall_risk === 'low'
                                                            ? <ShieldCheck size={20} className="text-green-500 shrink-0" />
                                                            : <ShieldAlert size={20} className="text-amber-500 shrink-0" />
                                                        }
                                                        <div className="flex-1">
                                                            <div className="flex justify-between text-xs mb-1">
                                                                <span className="font-semibold text-slate-700">Fairness Score</span>
                                                                <span className={cn(
                                                                    'font-bold',
                                                                    biasAudit.fairness_score >= 80 ? 'text-green-600'
                                                                    : biasAudit.fairness_score >= 55 ? 'text-amber-600'
                                                                    : 'text-red-600',
                                                                )}>
                                                                    {biasAudit.fairness_score}/100 ({biasAudit.overall_risk} risk)
                                                                </span>
                                                            </div>
                                                            <div className="h-1.5 bg-slate-200 rounded-full overflow-hidden">
                                                                <div
                                                                    className={cn(
                                                                        'h-full rounded-full',
                                                                        biasAudit.fairness_score >= 80 ? 'bg-green-500'
                                                                        : biasAudit.fairness_score >= 55 ? 'bg-amber-400'
                                                                        : 'bg-red-500',
                                                                    )}
                                                                    style={{ width: `${biasAudit.fairness_score}%` }}
                                                                />
                                                            </div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    {/* Flags */}
                                                    {biasAudit.bias_flags.length > 0 && (
                                                        <div className="space-y-1.5 max-h-44 overflow-y-auto">
                                                            {biasAudit.bias_flags.map((flag: BiasFlag, i: number) => (
                                                                <div key={i} className={cn(
                                                                    'text-[11px] rounded p-2 border space-y-0.5',
                                                                    flag.risk_level === 'high' ? 'bg-red-50 border-red-200 text-red-700'
                                                                    : flag.risk_level === 'medium' ? 'bg-amber-50 border-amber-200 text-amber-700'
                                                    ...
                                                    {biasAudit.bias_flags.length === 0 && (
                                                        <p className="text-xs text-green-700 italic">
                                                            No bias signals detected in this candidate/job pair.
                                                        </p>
                                                    )}

                                                    {biasAudit.requires_blind_review && (
                                                        <p className="text-[11px] font-semibold text-red-700 flex items-center gap-1">
                                                            <ShieldAlert size={12} />
                                                            Blind review recommended before finalizing this decision.
                                                        </p>
                                                    )}
                                                </div>
                                            )}
